...
export async function discoverMdns(listenMs: number, localAddresses: string[]): Promise<MdnsRecord[]> {
    ...
    for (const rr of all) {
      switch (rr.type) {
        case TYPE.PTR: {
          ...
          if (target && !entry.name) entry.name = prettyInstance(target);
          break;
        }
        case TYPE.SRV: {
          ...
          if (!entry.name && rr.name) entry.name = prettyInstance(rr.name);
          break;
        }
        case TYPE.TXT: {
          for (const kv of (rr.data as string[]) ?? []) {
            ...
            if (value && !entry.txt[key]) entry.txt[key] = value;
          }
          break;
        }
        case TYPE.A: {
  ...
  const results = [...byIp.values()].filter((r) => !localAddresses.includes(r.ip));
